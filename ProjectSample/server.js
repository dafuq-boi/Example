...
const mongourl = 'mongodb://ac-buwtvqp-shard-00-00.w8d8uyb.mongodb.net:27017/?authSource=admin&compressors=none&replicaSet=atlas-148yfn-shard-0&ssl=true'; 
const dbName = 'test';
...
const createDocument = function(db, createddocuments, callback){
    ...
    client.connect(function(err) {
        ...
        db.collection('restaurant').insertOne(createddocuments, function(error, results){
            if(error){
            	throw error
            };
            console.log(results);
...
const handle_Edit = function(res, criteria) {
    ...
    client.connect(function(err) {
        ...
        let cursor = db.collection('restaurant').find(documentID);
        cursor.toArray(function(err,docs) {
            client.close();
            assert.equal(err,null);
...
console.log(criteria);
	db.collection('restaurant').deleteOne(criteria, function(err, results){
	assert.equal(err, null);
	console.log(results);
	return callback();
	});
...
app.post('/api/item/restaurantID/:restaurantID', function(req,res) {
    if (req.params.restaurantID) {
   	...
   	db.collection('restaurant').insertOne(newDocument, function(err,results){
                assert.equal(err,null);
                client.close()
                res.status(200).end()
                    });
...
app.delete('/api/item/restaurantID/:restaurantID', function(req,res){
    if (req.params.restaurantID) {
        ...
        client.connect(function(err){
            ...
            db.collection('restaurant').deleteMany(criteria, function(err,results) {
                assert.equal(err,null)
                client.close()
                res.status(200).end();
            })
